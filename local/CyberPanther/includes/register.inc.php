...
if (isset($_POST['register-submit'])) {
    ...
    if (empty($username) || empty($email) ||empty($studentID) ||empty($password) ||empty($passwordRepeat)) {        //if all fields are empty 
        header("Location: ../login.html?error=emptyfields");                                                      //calls empty fields error from register file
        exit();
    }
        ...
        header("Location: ../login.html?error=invalidemailuid");                                                  //call error from register file
        exit();
    }
    else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {                          // if the email is not valid
        header("Location: ../login.html?error=invalidemail");                     //call invalid email from register file 
        exit();
    }
    else if (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {                          //if the username is not valid 
        header("Location: ../login.html?error=invaliduid");                       //call invalid username from register file 
        exit();
    }
    else if ($password !== $passwordRepeat) {                                       //if the password is not equal to the the repeated password
        header("Location: ../login.html?error=passwordcheck&uid=" .$username. "&mail=" .$email);                    //calls passwords are not the same error from file
        exit();
    }
    else {

        $sql = "SELECT uidUSERS FROM users WHERE uidUsers=? AND sidUsers=?";                        //$sql = "SELECT uidUSERS FROM users WHERE uidUsers=? AND sidUsers=?";  $sql = "SELECT * FROM users WHERE uidUsers=?;";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($resultCheck > 0) {                                                     //if there is more than one occurance of the parameters
                header("Location: ../register.index.html?error=uidtakensidtaken");             //call error username or id is taken 
                exit();
            }
            else {
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)) {                                                //if sql statement cannot prepare for execution     
                    header("Location: ../login.html?error=sqlerror");                                 //call sql error from file 
                    exit();
                }
                else {
                    ...
                    mysqli_stmt_execute($stmt);                                                                 //
                    header("Location: ../login.html");                                        //calls success message from file 
                    exit();
                }
