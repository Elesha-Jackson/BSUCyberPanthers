...
if (isset($_POST['login-submit'])) {
    ...
    if (empty($mailuid) || empty($password)) {                  
        header("Location: ../login.html?error=emptyfields");                         //error checking for no input
        exit();
    }
    else {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {                                    //if the connection is succes 
            header("Location: ../login.html?error=sqlerror");                        //calls error from                            
            exit();                                                                 //exit code
        }
        else {
            ...
            if ($row = mysqli_fetch_assoc($result)) {                              //if result is found in row
                ...
                if ($pwdCheck == false) {                                          //if not found in table 
                    header("Location: ../login.html?error=wrongpassword");          //calls error from file 
                    exit();                                                        //exit code
                    ...
                    header("Location: ../bag.html?login=success");                //call success from index file 
                }
                else {
                    header("Location: ../login.html?error=wrongpwd");                //calls wrong password error 
                    exit();                                                         //exit
                }
            }
            else {
                header("Location: ../login.html?error=nouser");                      //user doesnt exits error from index file 
        exit();
            }
        }
    }
